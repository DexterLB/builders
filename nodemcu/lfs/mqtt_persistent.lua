...
local function connect(subscriptions, connected, disconnected, lwt)
    ...
    local obj = {
        publish = function(self, topic, msg, qos, retain)
            ...
            self.current_mqtt_client:publish(MQTT_ROOT .. topic, msg, qos, retain)
        end,
    ...
    for topic,sub in pairs(subscriptions) do
        local full_topic = MQTT_ROOT .. topic
    ...
    if lwt ~= nil then
        mqtt_client:lwt(MQTT_ROOT .. lwt[1], lwt[2], 1, 1)
    end
    ...
    local connect_mqtt = function()
        ...
        local ok = mqtt_client:connect(MQTT_HOST, MQTT_PORT, is_tls,
            function(client)
